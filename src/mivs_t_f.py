...
def make_algos(n):
    ...
    """
    return [
        (f"OPLLGA(sqrt(log n))", lambda: OPLLGA(n, int(sqrt(log(n))), int(sqrt(log(n)))), "#FFF943", r"$(1+ \lambda, \lambda)$-GA"),
        (f"UMDA(sqrt(n)·log n)", lambda: UMDA(n, int(sqrt(n) * log(n)), int(log(n))), "#FF9643", r"UMDA"),
        ("TPOGA",                lambda: TPOGA(n), "#FF435D", r"$(2+1)$-GA"),
        (f"DEGA_Limit(n^(2/3))", lambda: DEGA_Limit(n, int(n**(2/3))), "#D443FF", r"$DEGA^{\lambda=n^{2/3}}$"),
        ("DEGA_A",               lambda: DEGA_A(n), "#7143FF", r"$DEGA_+$"),
        ("DEGA_B",               lambda: DEGA_B(n), "#43CCFF", r"$DEGA_{LO}$"),
        (f"DEGA_Limit(log n)",   lambda: DEGA_Limit(n, int(log(n))), "#43FF76", r"$DEGA^{\lambda=\log n}$"),
    ]
...
# ------------------------
out_path = "./plots/anytime_mivs_new.pdf"
...
with PdfPages(out_path) as pdf:
    ...
    color_map = {label: prop_colors[i % len(prop_colors)] for i, label in enumerate(all_labels)}
    ...
    for i in range(4):
        ...
        if i < len(N_LIST):
            ...
            ax.set_title(f"MIVS Anytime Performance (n={n})")
    ...
    if legend_handles:
        ...
        fig.legend(handles, labels, loc="lower center", bbox_to_anchor=(0.5, 0.03), ncol=7, fontsize="medium", frameon=False)
